...
Attempted request counts remain unknown until coordinator integration.
"""
...
JOURNAL_SCHEMA_VERSION: Final = "1.1"
...
class RunJournal:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            self._publish("run_started.json", content, lambda: None)
    ...
    def start(cls, *, output_root: str | Path, run_id: str,
              ...
              pagination_policy: str = DEFAULT_PAGINATION_POLICY) -> "RunJournal":
        return cls(output_root, run_id, expected_offsets, started_at, api_key, pagination_policy)
    ...
    def append_page(self, record: PageJournalRecord) -> Path:
        self._ensure_open()
        ...
        return self._publish(f"page_{index:06d}.json", content, lambda: self._pages.append(record))

    def complete(self, *, completed_at: datetime, stop_reason: str,
                 observed_number_matched: int | None,
                 duplicate_id_check_complete: bool) -> Path:
        self._ensure_open()
        ...
        return self._publish("run_complete.json", content, lambda: self._set_terminal("complete"))

    def fail(self, *, failed_at: datetime, failure_category: str) -> Path:
        self._ensure_open()
        ...
        return self._publish("run_failed.json", content, lambda: self._set_terminal("failed"))

    def _set_terminal(self, status: str) -> None:
        self._status = status
        self._terminal = True
